...
function CronTab(u, cb) {
  ...
  this.save = function(cb) {
    ...
    if (!_.isFunction(cb)) {
      exec('echo "'+ this.render() + '" | '+ COMMAND + ' ' + args);
      return self;
    }
...
exports.load = function(username, callback) {
  if (_.isString(username) && _.isFunction(callback)) {
    ...
    if (_.isString(username)) {
      if (!exec('id -u '+ username).toString(), { stdio: 'pipe' }) {
...
exports.load.sync = function (username, callback) {
  return exports.load(username, callback);
};
