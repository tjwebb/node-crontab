...
function CronTab(u, cb) {
  ...
  this.save = function(cb) {
    ...
    if (!_.isFunction(cb)) {
      var result = exec('echo "'+ this.render() + '" | '+ COMMAND + ' ' + args);
      if (result.status !== 0) {
        throw new Error(JSON.stringify({ message: result.stdout }));
      }
      return self;
    }
...
exports.load = function(username, callback) {
  if (_.isString(username) && _.isFunction(callback)) {
    ...
    if (_.isString(username)) {
      if (exec('id -u '+ username).status !== 0) {
        throw new Error('user does not exist on system: '+ username);
      }
